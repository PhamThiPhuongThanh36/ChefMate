...
fun HomeScreen(recipeViewModel: RecipeViewModel, navController: NavController) {
    ...
    Column(
        ...
        LaunchedEffect(Unit) {
            recipes = recipeViewModel.getTopTrending()
            recipes.forEachIndexed { i, r ->
                Log.d("CheckRecipe", "[$i] name=${r.recipeName}, img=${r.image}, cook=${r.cookingTime}")
            }
        }
        Log.d("TopTrending", "recipess: $recipes")
        ...
        LazyColumn(
            ...
            items(recipes.size) { index ->
                BigCard(
                    ...
                    onClick = { navController.navigate("recipeDetail/${recipes[index].recipeId}") },
                    modifier = Modifier
                        .fillMaxWidth(0.9f)
                )
            }
